...
export default function shootRandom(start, delay, end, sum, append, power = 10, startAngle = 0, endAngle = 359, addAngle = 0, max = 1, spy = false, ...arg) {
    let step = 0, count = 0
    return (inst) => {
        if (step > end) {
            return true
        ...
        if (step > start && step < end && step % delay === 0 && count < max) {
            ...
            for (let i = startAngle; i <= endAngle; i += (endAngle - startAngle) / sum) {
                for (let k = 0; k < append; k++) {
                    ...
                    speed = transTo(speed, 0, spyRad + ((i + 45 + ((Math.nextSeed() - 0.5) * power)) % 360) * L);
