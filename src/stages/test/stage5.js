...
export const testStage5 = () => {
    return new StageItem(new Map([[0, function () {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            entities.push(Kedama(color, GUI_SCREEN.WIDTH + i * 30, GUI_SCREEN.Y + 100 * Math.nextSeed(), 1,
                batchExecute([shootDot(3 + i * 4, 10, 120 + i * 4, 1,
                    true, 'small', color, 0, 0, 0, 4), moveLine(-4, 0)]),
                300, [BlueOrb(0, 0, 0, -1)]))
        }
        for (let i = 0; i < 10; i++) {
            const color = i % 2 === 0 ? 'blue' : 'green'
            entities.push(Kedama(color, GUI_SCREEN.X - i * 30, GUI_SCREEN.Y + 100 * Math.nextSeed(), 1,
                batchExecute([shootDot(3 + i * 4, 10, 120 + i * 4, 1,
                    true, 'small', color, 0, 0, 0, 4), moveLine(4, 0)]),
                300, [BlueOrb(0, 0, 0, -1)]))
        }
